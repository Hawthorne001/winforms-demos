...
namespace TreeViewAdvDragDrop
{
	...
	public class Form1 : SfForm
	{
        ...
        private void treeViewAdv_DragDrop(object sender, System.Windows.Forms.DragEventArgs e)
		{
            ...
            // ES - 989460 - If the source node and the destination node are the same,
            // skip setting the selected node to avoid an ArgumentException.
            if (sourceNode.NodeStyle.Data.ToString() != destinationNode.NodeStyle.Data.ToString())
		...
		{
            // ES-989460 - If the source node and the highlighted node are the same, allow dropping only above or below the highlighted node.
            // Otherwise, allow dropping above, below, or on the highlighted node.
            bool isSameNode = e.HighlightNode.NodeStyle.Data?.ToString() == this.currentSourceNode.NodeStyle.Data?.ToString();

            e.AllowedPositions = isSameNode ? TreeViewDropPositions.AboveNode | TreeViewDropPositions.BelowNode : TreeViewDropPositions.All;

            this.treeViewDragHighlightTracker.EdgeSensitivityOnTop = e.HighlightNode.Bounds.Height / (isSameNode ? 2 : 4);
        ...
        private void TreeDragDrop_QueryAllowedPositionsForNode1(object sender, QueryAllowedPositionsEventArgs e)
        {
            ...
            this.treeViewDragHighlightTracker1.EdgeSensitivityOnTop = e.HighlightNode.Bounds.Height / (isSameNode ? 2 : 4);
